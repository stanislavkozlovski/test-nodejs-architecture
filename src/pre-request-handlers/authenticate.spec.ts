...
describe("authenticate", () => {
  ...
  it("results in an error if the token has an unexpected structure", async () => {
    const unexpectedToken: jwt.Jwt = {
      payload: {},
      header: {
        kid: "kid",
        alg: "RS256",
    ...
    jest.spyOn(jwt, "decode").mockReturnValue(unexpectedToken);
    jest.spyOn(jwt, "verify").mockReturnValue();
    await authenticate(request, response, next);
    expect(response.status).not.toHaveBeenCalled();
